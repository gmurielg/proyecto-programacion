...
def video(estados,fps,dt,l):
    ...
    for i in range(a):
        if i%(1/(dt*fps))==0:
            theta=estados[i,0]
            img=pendulo(250,250,l,theta)
            frames.append(img)
    
    frames[0].save("C:/Users/janus/Desktop/pendulo_simple.gif",save_all=True,append_images=frames[1:],duration=40,laoop=0)
    
video(trayectoria(np.pi/3,0,10000,0.001,15),20,0.001,50)
